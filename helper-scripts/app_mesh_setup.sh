...
for PROFILE in shared frontend backend
    do 
        ...
        if [ "$PROFILE" != "shared" ] && [ "$1" != "org" ]; then 
            echo "Accepting resource share..."
            aws --profile $PROFILE ram accept-resource-share-invitation \
              --resource-share-invitation-arn $(aws --profile $PROFILE ram get-resource-share-invitations \
              | jq -r '.resourceShareInvitations[] | select(.resourceShareName=="mesh-share") | .resourceShareInvitationArn') > /dev/null
        fi 
